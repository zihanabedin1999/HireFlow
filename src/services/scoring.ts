...
export class ScoringService {
  ...
  async scoreCandidates(
    ...
    for (const candidate of candidates) {
      ...
      // Score out of 1000 (raw matches)
      const rawScore = matched;
      // Score out of 1000 for display
  ...
  private async scoreCandidate(
    ...
    try {
      ...
      logger.error('OpenAI scoring failed:', error);
      // Return a default low score if scoring fails
      return this.createDefaultScore(candidate);
    }
  }

  private buildScoringPrompt(candidate: Candidate, jobDescription: JobDescription): string {
    return `
...
`;
  ...
  private parseScoringResponse(candidate: Candidate, response: string): ScoredCandidate {
    try {
  ...
  private createDefaultScore(candidate: Candidate): ScoredCandidate {
    return {
      ...candidate,
      score: 5,
      confidence: 0.3,
  ...
  async getTopCandidates(
    ...
    return candidates
      .filter(c => c.score >= 6 && c.confidence >= 0.5) // Filter by minimum quality
