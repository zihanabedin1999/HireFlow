...
exports.LinkedInSearchService = void 0;
...
class LinkedInSearchService {
    ...
    async searchCandidates(jobDescription, location) {
    ...
    async searchCustomDatabase(jobDescription, location) {
        ...
        logger_1.default.info(`Found ${matchingCandidates.length} candidates with 2%+ match for job description`);
        return matchingCandidates.map(item => item.candidate);
    }
    calculateMatchScore(candidate, jobDescription, location) {
        const skillKeywords = [
    ...
    calculateTitleMatch(candidateHeadline, jobDescription) {
    ...
    calculateSkillsMatch(candidateSkills, jobDescription) {
        ...
        for (const jobSkill of jobSkills) {
            if (candidateSkillsLower.some(skill => skill.includes(jobSkill) ||
    ...
    areSimilarSkills(skill1, skill2) {
        ...
        for (const [mainSkill, variations] of Object.entries(skillMappings)) {
            if (variations.includes(skill1) && skill2.includes(mainSkill))
                return true;
            if (variations.includes(skill2) && skill1.includes(mainSkill))
                return true;
        }
        return false;
    }
    ...
    calculateExperienceMatch(candidateExperience, jobDescription) {
        ...
        if (description.includes('senior') || description.includes('lead') || description.includes('principal')) {
            if (experience.includes('8+') || experience.includes('9+') || experience.includes('10+'))
                return 1.0;
            if (experience.includes('6+') || experience.includes('7+'))
                return 0.8;
            if (experience.includes('4+') || experience.includes('5+'))
                return 0.6;
            return 0.3;
        }
        if (description.includes('mid') || description.includes('intermediate')) {
            if (experience.includes('4+') || experience.includes('5+') || experience.includes('6+'))
                return 1.0;
            if (experience.includes('3+') || experience.includes('7+'))
                return 0.7;
            return 0.4;
        }
        if (description.includes('junior') || description.includes('entry')) {
            if (experience.includes('1+') || experience.includes('2+') || experience.includes('3+'))
                return 1.0;
            if (experience.includes('4+') || experience.includes('5+'))
                return 0.6;
            return 0.3;
    ...
    async getCandidateDetails(linkedinUrl) {
        return customCandidates.find(c => c.linkedinUrl === linkedinUrl) || null;
    }
}
exports.LinkedInSearchService = LinkedInSearchService;
exports.default = new LinkedInSearchService();
